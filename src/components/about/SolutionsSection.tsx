    ...
    return (
      <div key={index} className={cn("flex flex-col", extraClass)}>
        <button
          ...
          className="relative w-full bg-white rounded-xl lg:rounded-2xl p-4 lg:p-8 text-center hover:shadow-lg transition-all duration-300 group border border-gray-200 h-full flex flex-col items-center pointer-events-auto min-h-[160px] lg:min-h-[240px]"
        >
          <div className="absolute top-3 right-3">
            <Maximize2 className="w-4 h-4 opacity-40 group-hover:opacity-100 transition-opacity" style={{ color: "#9ca3af" }} />
          </div>
          ...
          <h3 className="text-sm lg:text-base font-semibold mb-1" style={{ color: BLUE }}>
            {solution.title}
          </h3>
          <p className="text-xs lg:text-sm" style={{ color: "#6b7280" }}>
            {solution.subtitle}
          </p>
        </button>
    ...
    <section ref={ref} className="py-16 md:py-24 px-4">
      <div className="max-w-5xl mx-auto">
        <div className={cn("text-center mb-10 transition-all duration-700", isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8")}>
          <h2 className="text-2xl md:text-4xl font-bold italic" style={{ color: BLUE }}>
            Our solutions
          </h2>
        </div>
        ...
        <p className={cn("text-center text-sm mt-12 transition-all duration-700 delay-400", isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8")} style={{ color: "#6b7280" }}>
