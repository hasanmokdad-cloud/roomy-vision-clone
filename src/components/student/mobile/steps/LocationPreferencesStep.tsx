...
const LocationPreferencesStep = ({ data, onChange }: LocationPreferencesStepProps) => {
  // Convert areas to value/label format for the dropdown
  const getAreasForCity = (cityValue: string) => {
    const areas = areasByCity[cityValue] || [];
  ...
  const areas = data.city ? getAreasForCity(data.city) : [];
